...
TokenRefresh.refresh = async (req, res, next) => {
  ...
  if (!token) {
    ...
    if (Object.keys(cookie).length && Object.prototype.hasOwnProperty.call(cookie, 'X-Token')) {
  ...
  if (!token) {
    ...
    try {
      ...
      if ((expiration - currTime) < 300) {
        ...
        const refreshToken = jwt.sign({id: decoded.id, school : decoded.school}, 'ABCD', { expiresIn: '7d' });
